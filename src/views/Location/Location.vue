      ...
      <div v-for="location in result.locations.results" :key="location.id">
        <div v-if="location.id == id">
          <div class="mb-6 text-center">
            <h2 class="mb-2 text-3xl font-bold">{{ location.name }}</h2>
            <p class="text-lg text-gray-400">{{ location.type }}</p>
            <p class="text-lg text-gray-400">{{ location.dimension }}</p>
            <p class="text-sm text-gray-500">Created: {{ location.created }}</p>
          </div>
          <div class="mt-8">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              <div v-for="character in location.residents" :key="character.id" class="flex p-4 bg-gray-800 rounded-lg shadow-lg">
                <img :src="character.image" alt="Character Image" class="object-cover w-24 h-24 mr-4 rounded-full">
                <div>
